...
3.11–3.14 matrix. `RELEASE-003 — Run tests, Black, and Ruff in CI` added
repository-wide formatting and lint enforcement to every matrix job. The
workflow uses independent jobs with fail-fast disabled, so a failure on one
interpreter does not hide results from the others.
...
foreach ($version in $versions) {
    ...
    & "$environment\Scripts\python.exe" -m black --check $projectRoot
    & "$environment\Scripts\python.exe" -m ruff check $projectRoot
    & "$environment\Scripts\python.exe" -m pytest -q $projectRoot
}
```

Every environment should report clean Black and Ruff checks, `351 passed`, and
